...
impl MapIterator {
    pub fn new(tokens: Rc<Vec<LumpToken>>) -> Self {
        Self {
            tokens,
            current_index: 0,
        }
    ...
    fn next(&mut self) -> Option<Self::Item> {
        if self.tokens.is_empty() {
            ...
            match self.tokens.get(self.current_index) {
                Some(LumpToken::MapMarker(name)) => {

                    let mut map = Map::new(name.clone());
                    self.current_index += 1;

                    while let Some(token) = self.tokens.get(self.current_index) {
                        match token {
                            LumpToken::Lump(lump_name, lump_ref) => match lump_name.as_str() {
